...
class ReservationService {
	...
	async submit(payload) {
			...
			fetch(
				...
				.then((response) => {
					if (response.success) {
						...
						reservation = new this.db.Reservation(
							reservationData,
						);
					} else {
						okToken = false;
		...
		try {
			...
			if (payload.token) {
				fetch(
					'https://ip-accounts.herokuapp.com/api/clients/addReservation',
					{
						method: 'POST',
						body: {
	...
	async checkRestaurantAvailability(reservationData) {
		...
		let reservationDay;
		await fetch(
			`https://ip-accounts.herokuapp.com/api/users/${reservationData.restaurantId}`,
		)
